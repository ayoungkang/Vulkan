...
const float EYEDIST =		12.;			// eye distance
...
main(int argc, char* argv[])
{
	...
	for (int i = 0; i < NUMCURVES; i++)
	{
		...
		Curves[i].p0.x0 = (float)(0.3 * cos((2. * i * M_PI / 12.)));
		Curves[i].p0.x = (float)(0.3 * cos((2. * i * M_PI / 12.)));
		Curves[i].p0.y0 = (float)(0.3 * sin((2. * i * M_PI / 12.)));
		Curves[i].p0.y = (float)(0.3 * sin((2. * i * M_PI / 12.)));
		Curves[i].p0.z0 = 0.;
		Curves[i].p0.z = 0.;

		Curves[i].p3.x0 = (float)(0.3 * cos((2. * (i + 1) * M_PI / 12.)));
		Curves[i].p3.x = (float)(0.3 * cos((2. * (i + 1) * M_PI / 12.)));
		Curves[i].p3.y0 = (float)(0.3 * sin((2. * (i + 1) * M_PI / 12.)));
		Curves[i].p3.y = (float)(0.3 * sin((2. * (i + 1) * M_PI / 12.)));
		Curves[i].p3.z0 = 0;
		Curves[i].p3.z = 0;

		Curves[i].p1.x0 = (float)(2.3 * cos((2. * i * M_PI / 12.)));
		Curves[i].p1.x = (float)(2.3 * cos((2. * i * M_PI / 12.)));
		Curves[i].p1.y0 = (float)(2.3 * sin((2. * i * M_PI / 12.)));
		Curves[i].p1.y = (float)(2.3 * sin((2. * i * M_PI / 12.)));
		Curves[i].p1.z0 = 0.5;
		Curves[i].p1.z = 0.5;

		Curves[i].p2.x0 = (float)(2.3 * cos((2. * (i + 1) * M_PI / 12.)));
		Curves[i].p2.x = (float)(2.3 * cos((2. * (i + 1) * M_PI / 12.)));
		Curves[i].p2.y0 = (float)(2.3 * sin((2. * (i + 1) * M_PI / 12.)));
		Curves[i].p2.y = (float)(2.3 * sin((2. * (i + 1) * M_PI / 12.)));
		Curves[i].p2.z0 = -0.5;
		Curves[i].p2.z = -0.5;
	}
	...
	for (int i = 0; i < NUMCURVES; i++)
	{
		...
		p0[1] = Curves[i].p0.y;
		p0[2] = Curves[i].p0.z;
		VertexData2[j].position = glm::make_vec3(p0);
		VertexData2[j].color = glm::make_vec3(color);

		p1[0] = Curves[i].p1.x;
		p1[1] = Curves[i].p1.y;
		p1[2] = Curves[i].p1.z;
		VertexData2[j + 1].position = glm::make_vec3(p1);
		VertexData2[j + 1].color = glm::make_vec3(color);

		p2[0] = Curves[i].p2.x;
		p2[1] = Curves[i].p2.y;
		p2[2] = Curves[i].p2.z;
		VertexData2[j + 2].position = glm::make_vec3(p2);
		VertexData2[j + 2].color = glm::make_vec3(color);

		p3[0] = Curves[i].p3.x;
		p3[1] = Curves[i].p3.y;
		p3[2] = Curves[i].p3.z;
		VertexData2[j + 3].position = glm::make_vec3(p3);
		VertexData2[j + 3].color = glm::make_vec3(color);

		//cout << glm::to_string(VertexData[j].position) << endl;
		//cout << glm::to_string(VertexData[j + 1].position) << endl;
		//cout << glm::to_string(VertexData[j + 2].position) << endl;
		//cout << glm::to_string(VertexData[j + 3].position) << endl;
	}
	...
	for (int i = 0; i < NUMCURVES; i++) 
	{
		...
		for (int it = 0; it <= NUMPOINTS; it++)
		{
			...
			p0[1] = y;
			p0[2] = z;
			VertexData[i*NUMPOINTS + it].position = glm::make_vec3(p0);
			p1[0] = rgb[0];
			p1[1] = rgb[1];
			p1[2] = rgb[2];
			VertexData[i * NUMPOINTS + it].color = glm::make_vec3(p1);
			//cout << glm::to_string(VertexData[i * NUMPOINTS + it].position) << endl;
		}
...
InitGraphics( )
{
	...
	Init05MyVertexDataBuffer(sizeof(VertexData2), &MyVertexDataBuffer2);
	Fill05DataBuffer(MyVertexDataBuffer2, (void*)VertexData2);
	...
	Init14GraphicsVertexFragmentPipeline( ShaderModuleVertex, ShaderModuleFragment, VK_PRIMITIVE_TOPOLOGY_LINE_STRIP, &GraphicsPipeline  );
	Init14GraphicsVertexFragmentPipeline(ShaderModuleVertex, ShaderModuleFragment, VK_PRIMITIVE_TOPOLOGY_POINT_LIST, &GraphicsPipeline2);
...
VK_FORMAT_X = VK_FORMAT_R32_SFLOAT
	...
	VkPipelineInputAssemblyStateCreateInfo			vpiasci;
		...
		vpiasci.flags = 0;
		//vpiasci.topology = VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
		vpiasci.topology = topology;
#ifdef CHOICES
...
RenderScene( )
{
	...
	// all 3 buffer for geometry:
		...
		VkBuffer buffers2[1] = { MyVertexDataBuffer2.buffer };
	...
	const uint32_t vertexCount2 = sizeof(VertexData2) / sizeof(VertexData2[0]);

	//printf("%d\n", vertexCount2);
	//cout <<  glm::to_string(VertexData[0].position) << endl;
	...
	{
			vkCmdSetLineWidth(CommandBuffers[nextImageIndex], 3.);
        	vkCmdDraw( CommandBuffers[nextImageIndex], vertexCount, instanceCount, firstVertex, firstInstance );
			
			if (UseControlLine)
			{
				vkCmdBindVertexBuffers(CommandBuffers[nextImageIndex], 0, 1, buffers2, offsets);
				vkCmdSetLineWidth(CommandBuffers[nextImageIndex], 1.);
				vkCmdDraw(CommandBuffers[nextImageIndex], vertexCount2, instanceCount, firstVertex, firstInstance);
			}
			
	}

	vkCmdBindPipeline(CommandBuffers[nextImageIndex], VK_PIPELINE_BIND_POINT_GRAPHICS, GraphicsPipeline2);
	if (UseControlPoint)
	{
		vkCmdBindVertexBuffers(CommandBuffers[nextImageIndex], 0, 1, buffers2, offsets);
		vkCmdDraw(CommandBuffers[nextImageIndex], vertexCount2, instanceCount, firstVertex, firstInstance);
...
Reset( )
{
	...
	UseLighting = false;
	UseRotate = false;
	UseControlLine = false;
	UseControlPoint = false;
	Frozen = false;
	Verbose = true;
	Xrot = Yrot = 0.;
...
UpdateScene( )
{
	...
	int ms = int(Time);
	ms %= 100;
	Misc.uTime2 = (float)ms / (float)100;
...
GLFWKeyboard( GLFWwindow * window, int key, int scancode, int action, int mods )
{
	if( action == GLFW_PRESS )
	{
		switch( key )
		{
			...
			case 'c':
			case 'C':
				UseControlLine = !UseControlLine;
				break;

			case 'd':
			case 'D':
				UseControlPoint = !UseControlPoint;
				break;
